...
class Guy {
	...
	Guy( Terrain* terrain1, float terrainScale1) 
	{
	    ...
	    z0=0.0f;
	    y0=0.375;
	    //y0=terrainScale*heightAt(terrain, x0 / terrainScale, z0 / terrainScale);
	    //pitch = 0.0f;
	}
	...
	void draw() 
	{
	    ...
	    glPushMatrix();
	    glColor3f(0.0,0.0,0.0);
	    glTranslatef(3.0f,-0.7f,0);
	    glRotatef(rolling,0.0f,0.0f,-1.0f);
	    glutWireTorus(0.4f,1.0f,30,30);
	    glPopMatrix();
	    glPushMatrix();
	    glColor3f(0.0,0.0,0.0);
	    glTranslatef(-2.0f,-0.7f,0);
	    glRotatef(rolling,0.0f,0.0f,-1.0f);
	    glutWireTorus(0.4f,1.0f,30,30);
	    glPopMatrix();
	    glPopMatrix();
	...
	void step() {
	    ...
	    {
		//cout<<fossil[i].first<<" "<<fossil[i].second<<" "<<x0<<" "<<z0<<endl;
...
void drawScene() 
{
	...
	char score[100];
	sprintf(score,"Score: %d",36-_fossils.size());
	printtext(-0.6,0.9,string(score));
	if(_fossils.size()==0)
	{
	    glClearColor(1.0f, 1.0f, 1.0f, 1.0f);
	    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	    sprintf(text,"Game Over\nScore: %d",_fossils.size());
	    printtext(.0f,.0f,string(text));
	    game=over;
	    glutSwapBuffers();
	    return;
